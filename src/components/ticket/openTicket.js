...
function formatChannelName(format, member, ticketNumber, category) {
  return format
    ...
    .replace('{category}', (category?.name || 'ticket').toLowerCase().replace(/[^a-z0-9]/g, ''))
    .slice(0, 100);
}

async function handleOpenButton(interaction) {
  const config = await GuildConfig.findOne({ guildId: interaction.guildId });
  if (!config) return interaction.reply({ content: '❌ Ticket system is not configured yet.', ephemeral: true });

  // Check max open tickets
  const openCount = await Ticket.countDocuments({ guildId: interaction.guildId, creatorId: interaction.user.id, status: 'open' });
  if (openCount >= config.behavior.maxOpenTickets) {
    return interaction.reply({ content: `❌ You already have ${openCount} open ticket(s). Maximum is **${config.behavior.maxOpenTickets}**.`, ephemeral: true });
  }
...
async function handleCategorySelect(interaction) {
  ...
  if (!category) return interaction.update({ content: '❌ Category not found.', components: [] });
...
async function openTicketFlow(interaction, config, category) {
  if (category?.questions?.length) {
    ...
    if (interaction.deferred || interaction.replied) {
      ...
      await interaction.followUp({ content: `${emojis.ticket} Opening your ticket in a moment...`, ephemeral: true });
...
async function createTicketChannel(interaction, config, category, answers) {
  ...
  await GuildConfig.updateOne({ guildId: guild.id }, { $inc: { ticketCounter: 1 } });

  const channelName = formatChannelName(
    config.behavior.namingFormat || 'ticket-{username}',
    member, ticketNumber, category,
  );
  ...
  if (interaction.deferred || interaction.replied) {
    await interaction.editReply({ content: replyContent, components: [] }).catch(() =>
      interaction.followUp({ content: replyContent, ephemeral: true }).catch(() => {}),
    );
  } else {
...
module.exports = { handleOpenButton, handleCategorySelect, handleQuestionsModal };
