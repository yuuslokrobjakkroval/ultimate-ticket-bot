...
async function closeTicketChannel(
  ...
  if (config?.channels?.logsChannelId && file) {
    const logsChannel = await guild.channels
      .fetch(config.channels.logsChannelId)
      .catch(() => null);
    if (logsChannel) {
      await logsChannel.send({
        embeds: [
          new EmbedBuilder()
            .setTitle(
              `${emojis.transcript} Transcript — Ticket #${ticket.ticketNumber}`,
            )
            .setColor("#5865F2")
            .addFields(
              {
                name: "Category",
                value: ticket.categoryName || "None",
                inline: true,
              },
              {
                name: "Opened by",
      ...
      const feedbackRow = config?.behavior?.feedbackEnabled
        ? [
            new ActionRowBuilder().addComponents(
              ...[1, 2, 3, 4, 5].map((n) =>
                new ButtonBuilder()
                  .setCustomId(`ticket:feedback:${ticket._id}:${n}`)
                  .setLabel("⭐".repeat(n))
                  .setStyle(ButtonStyle.Secondary),
              ),
            ),
  ...
  if (config?.behavior?.deleteOnClose) {
    await channel
      .delete(
        `Ticket #${ticket.ticketNumber} closed by ${closer?.tag || reason}`,
      )
      .catch(() => {});
  } else {
    ...
    if (config?.channels?.archiveCategoryId) {
      await channel
        .setParent(config.channels.archiveCategoryId, {
    ...
    await channel
      .send({
        embeds: [
          new EmbedBuilder()
            .setDescription(
              `${emojis.close} This ticket has been closed by ${closer ? `<@${closer.id}>` : reason}.`,
            )
            .setColor("#ED4245")
            .setTimestamp(),
        ],
...
async function handleFeedback(interaction, ticketId, rating) {
  ...
  if (
    config?.channels?.feedbackChannelId ||
    config?.channels?.feedbackLogChannelId
  ) {
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: "Claimed by",
          value: ticket.claimedById ? `<@${ticket.claimedById}>` : "Nobody",
          inline: true,
        },
      )
      .setTimestamp();
    ...
    if (config?.channels?.feedbackChannelId) {
      const feedbackChannel = await interaction.client.channels
        .fetch(config.channels.feedbackChannelId)
        .catch(() => null);
      if (feedbackChannel) await feedbackChannel.send({ embeds: [embed] });
    }
    if (config?.channels?.feedbackLogChannelId) {
      const feedbackLogChannel = await interaction.client.channels
        .fetch(config.channels.feedbackLogChannelId)
        .catch(() => null);
  ...
  await interaction.editReply({
    embeds: [
      new EmbedBuilder()
        .setDescription(
          `${"⭐".repeat(ticket.feedbackRating)} Thank you for your feedback!`,
        )
        .setColor("#fee75c"),
    ],
    components: [],
