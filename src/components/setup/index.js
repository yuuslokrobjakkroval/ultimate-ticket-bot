...
async function handle(interaction, parts) {
  ...
  if (action === "channels") {
    ...
    if (sub === "page")
      return showChannelSetup(interaction, null, parseInt(extra));
  ...
  if (action === "categories") {
    ...
    if (sub === "edit") {
      if (extra === "select")
        return openEditCategoryModal(interaction, interaction.values[0]);
    }
    if (sub === "delete") {
      if (extra === "select")
        return confirmDeleteCategory(interaction, interaction.values[0]);
