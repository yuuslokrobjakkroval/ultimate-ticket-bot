...
function buildStaffEmbed(config) {
  ...
  return new EmbedBuilder()
    ...
    .setDescription(
      "Set which roles can manage tickets and who gets pinged when a new ticket opens.",
    )
    .setColor("#5865F2")
    .addFields(
      {
      ...
      {
        name: "🎫 Ticket Open Role",
        value: s.ticketOpenRoleId ? `<@&${s.ticketOpenRoleId}>` : "*(none)*",
        inline: false,
      },
    );
}
...
function buildStaffRows() {
  return [
    ...
    new ActionRowBuilder().addComponents(
      new RoleSelectMenuBuilder()
        .setCustomId("setup:staff:ticketrole")
        .setPlaceholder("Select role to assign on ticket open (optional)")
        .setMinValues(0)
        .setMaxValues(1),
    ),
    new ActionRowBuilder().addComponents(
...
async function showStaffSetup(interaction, config) {
  ...
  await interaction.update({
    embeds: [buildStaffEmbed(config)],
    components: buildStaffRows(),
  });
}
...
async function handleStaffSelect(interaction, field) {
  ...
  else if (field === "ticketrole")
    config.staff.ticketOpenRoleId = values[0] || null;
